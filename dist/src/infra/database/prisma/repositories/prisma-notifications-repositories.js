...
let PrismaNotificationsRepository = class PrismaNotificationsRepository {
    ...
    async findById(notificationId) {
        ...
        const raw = prisma_notification_mapper_1.PrismaNotificationMapper.toDomain(notification);
        return raw;
    }
    ...
    async countByRecipientId(recipientId) {
    ...
    async save(notification) {
        const raw = prisma_notification_mapper_1.PrismaNotificationMapper.toPrisma(notification);
        await this.prismaService.notification.update({ where: { id: raw.id }, data: raw });
    }
};
